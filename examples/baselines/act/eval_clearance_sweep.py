...
class Args:
    ...
    control_mode_ttt: Optional[str] = None
    """Override control_mode for the force_head_ttt case only. Useful for giving TTT a more
    force-informative signal to adapt on (e.g. a softer/more compliant controller than the rigid
...
def evaluate_with_ttt(n, agent: Agent, eval_envs, norm_stats, args: Args, device, use_ttt: bool):
    ...
    if args.sim_backend == "physx_cpu" or args.sim_backend == "cpu":
        ...
        # stats (checkpoint's norm_stats) is built on CPU regardless of backend; move it to the
        # eval device once here, since GPU-backend envs return obs/action tensors already on
        # `device` and mixing devices in the arithmetic below errors out.
